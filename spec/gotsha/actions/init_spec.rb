  ...
  describe "init" do
    context "first time" do
      ...
      it "creates default files and Git configuration" do
        expect(File).to receive(:write).with(Gotsha::GL_CONFIG_FILE, File.read(Gotsha::GL_CONFIG_TEMPLATE_PATH))
    ...
    context "second time" do
      ...
      it "does not create any files; onle Git configuration" do
        expect(FileUtils).not_to receive(:mkdir_p).with(".gotsha/hooks")
